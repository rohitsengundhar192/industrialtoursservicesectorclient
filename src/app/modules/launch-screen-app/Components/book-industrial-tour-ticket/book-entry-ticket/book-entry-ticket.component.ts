...
@Component({
  selector: 'app-book-entry-ticket',
  templateUrl: './book-entry-ticket.component.html',
  styleUrls: ['./book-entry-ticket.component.scss'],
})
export class BookEntryTicketComponent implements OnInit {
  //* --------------------------  Start  -----------------------------------*//
  ...
  bookWow(val: any) {
    this._spiner.open();
    this._apiservice.getWOWTicketPrice(val).subscribe((res) => {
  ...
  openWOWWallet() {
    const dialogRef = this.dialog.open(WowWalletComponent, {
      disableClose: true,
      height: '500px',
      width: '360px',
      minWidth: '250px',
  ...
  radioBtnData(element: any) {
    ...
    this.selectedRadioUserId = element.user_id;
  }
  //* ------------------------------ Helper Function -----------------------*//
